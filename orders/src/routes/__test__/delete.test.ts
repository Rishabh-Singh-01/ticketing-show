...
it('can not accessed if the user is not signed in', async () => {
  const id = generateId();
  return request(app).delete(`${endpointUrl}/${id}`).send({}).expect(401);
});
...
it('returns 401 error if the order is not of the current user', async () => {
  ...
  await request(app)
    .delete(`${endpointUrl}/${order.id}`)
    .set('Cookie', userTwo)
    .send()
    .expect(401);
});

it('returns the cancelled order for the valid request', async () => {
  const ticket = await createTicket();
...
it('emits the event for cancelled order', async () => {
  const ticket = await createTicket();
